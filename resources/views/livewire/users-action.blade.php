				...
				<table class="table table-responsive w-full">
					<thead>
						<tr class="bg-gray-100">
							...
							<th class="px-4 py-2"><a wire:click.prevent="sortBy('id')" role="button" href="#">
								ID
							...
							<th class="px-4 py-2"><a wire:click.prevent="sortBy('email')" role="button" href="#">
								Email
								@include('includes._sort-icon', ['field' => 'email'])
							</a></th>
							
							<th class="px-4 py-2"><a wire:click.prevent="sortBy('organization_id')" role="button" href="#">
								Organization Name
								@include('includes._sort-icon', ['field' => 'organization_id'])
							</a></th>
					...
					<tbody>
						...
						<tr>
							...
							<td class="border px-4 py-2">{{ @($user->organizations->name) }}</td>
							<td class="border px-4 py-2">{{ $user->created_at ? $user->created_at->format('m/d/Y') : '' }}</td>
							<td class="border px-4 py-2">
								<button wire:click="updateShowModal({{ $user->id }})">
									<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg>
								</button>
								
								<button wire:click="deleteShowModal({{ $user->id }})">
